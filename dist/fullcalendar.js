...
$.extend(TimeGrid.prototype, {
    ...
    renderSegTable: function (segs, tableEl) {
        var trEl = tableEl.find('tr');
        var segCols;
        var i, seg;
        var col, colSegs;
        var containerEl;
        var currentHeight, outerHeight;
        ...
        for (i = 0; i < segs.length; i++) {
            seg = segs[i];
            currentHeight = seg.bottom - seg.top;
            outerHeight = seg.el.outerHeight();
            delete seg.backwardCoord;
            delete seg.forwardCoord;
            delete seg.forwardPressure;
            delete seg.forwardSegs;
            delete seg.level;
            if (outerHeight > currentHeight) {
                /*
                 * col: 0, el: n.fn.init[1], end: h, event: Object, isEnd: true, isStart: true, start: h
                 * backwardCoord: 0, bottom: 458.1666666666667, col: 0, el: n.fn.init[1], end: h, event: Object, forwardCoord: 1, forwardPressure: 0, forwardSegs: Array[0], isEnd: true, isStart: true, level: 0, start: h, top: 455
                 */
                seg.bottom = seg.top + outerHeight;
            }
        }
    ...
    computeSegVerticals: function (segs) {
        ...
        for (i = 0; i < segs.length; i++) {
            seg = segs[i];
            seg.eventStartMS = seg.start.clone().stripZone();
            seg.eventDurationMS = seg.end - seg.start;
...
;;
/* Resource view for rendering multiple columns for same date
----------------------------------------------------------------------------------------------------------------------*/
...
function ResourceView(calendar) {
    ...
    this.timeGrid = new TimeGrid(this);
    this.timeGrid.eventsToSegs = function (events, intervalStart, intervalEnd) {
        intervalStart = that.start.clone().stripZone();
        intervalEnd = that.end.clone().stripZone();
        var segs = $.map(events, function (evt) {
            var eventStart = evt.start.clone().stripZone(); // normalize
            var eventEnd = that.calendar.getEventEnd(evt).stripZone(); // compute (if necessary) and normalize
            var rangeStart = eventStart.clone().stripZone();
            var rangeEnd = eventEnd.clone().stripZone();
            var seg = intersectionToSeg(rangeStart, rangeEnd, intervalStart, intervalEnd);
            if (seg) {
                seg.col = that.findWithAttr(that.calendar.options.resources, "id", evt.resourceId);
                seg.event = evt;
                return seg;
            }
        });
...
$.extend(ResourceView.prototype, {
    ...
    render: function (colCnt) {
        ...
        this.el.addClass('fc-agenda-view').html(html);
    ...
    cellToDate: function () {
        return t.start.stripTime().clone();
    },
